...
gradient_accumulation_steps = batch_tokens //(micro_batch_tokens * ddp_world_size)
steps = 1
...
for step in range(steps):
    ...
    for sub_step in tqdm(range(gradient_accumulation_steps)):
        ...
        if ddp: 
            model.require_backward_grad_sync = (sub_step == (gradient_accumulation_steps-1))
    ...
    t1 = time.time()
    tok_rate = (data.B * data.T * gradient_accumulation_steps * ddp_world_size) / (t1 - t0)
    if is_master_process: 
